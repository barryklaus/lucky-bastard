(() => {
  ...
  const LOOT = [
    ...
    {name:'Bar Fighter',icon:'🍺',type:'ability',desc:'Hidden ability: chair and bottle weapons hit harder.',apply:g=>g.unlock('BAR FIGHTER')},
  ...
  class Game{
    ...
    useEnvironment(f,o){const obj=this.objects.filter(x=>x.active&&!x.broken).sort((a,b)=>Math.abs(a.x-f.rig.x)-Math.abs(b.x-f.rig.x))[0];if(!obj)return;const name=obj.names[obj.kind]||'arena prop';this.thought(f,pick(['FUCK THIS PROP.','VIBE CHECK.','PROPERTY DAMAGE, FAM.']));obj.active=false;obj.broken=true;f.cool=.65;this.property+=125;this.burst(obj.x,this.floor-45,'#f3d13b',8);this.say(`${f.name} smashes the ${name}. It hurts the budget, not either fighter.`)}
    attack(f,o,blocked){
  ...
  $('#again').onclick=()=>location.reload();window.LB3={Game,Rig,ArenaObject,Particle,Confetti,ImpactText,WeaponProjectile,CrowdProjectile,WEAPONS,LOOT,SCENES,POSES,SKINS,COLORS,TRAITS,NAMES,CROWD_SHOUTS,show,pick,rnd,chance,clamp,circle,poly,limb,ctxRoundRect,prepareCanvas};readCreator();requestAnimationFrame(preview);
})();
