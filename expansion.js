(() => {
  ...
  const ENV_FINISH={backyard:'TRAMPOLINE TAX AUDIT',office:'PHOTOCOPIER PERFORMANCE REVIEW',buffet:'SNEEZE-GUARD SPECIAL',wedding:'CAKE-BASED ANNULMENT',birthday:'BOUNCY-CASTLE CATASTROPHE'};
  ...
  Game.prototype.updateDirector=function(){
    if(!this.access.director){this.c.style.transform='none';return}const gap=Math.abs(this.player.rig.x-this.enemy.rig.x);let mode='WIDE',scale=1,tx=0;if(this.player.dead||this.enemy.dead){mode='FINAL BLOW';scale=1.09}else if(this.player.grappled||this.enemy.grappled){mode='WRESTLING CAM';scale=1.075}else if(gap<190){mode='CLOSE CHAOS';scale=1.045}else if(this.player.flight||this.enemy.flight){mode='AIRBORNE LIABILITY';scale=1.025}this.cameraScale+=(scale-this.cameraScale)*.12;const mid=(this.player.rig.x+this.enemy.rig.x)/2;tx=clamp((640-mid)*.025,-12,12);this.c.style.transform=`translateX(${tx}px) scale(${this.cameraScale})`;if(mode!==this.cameraMode){this.cameraMode=mode;$('#directorBadge').textContent='🎬 DIRECTOR: '+mode}
  };
  ...
  Game.prototype.useEnvironment=function(f,o){const obj=this.objects.filter(x=>x.active).sort((a,b)=>Math.abs(a.x-f.rig.x)-Math.abs(b.x-f.rig.x))[0];oldUseEnvironment.call(this,f,o);if(obj){obj.broken=true;addEvent(this,`${f.name} weaponized ${obj.names[obj.kind]||'the furniture'}.`)}};
  ...
  Game.prototype.kill=function(v,killer,dmg,crit){
    ...
    const nearby=this.objects.find(o=>Math.abs(o.x-v.rig.x)<210);if(nearby&&chance(.48)){const title=ENV_FINISH[this.scene.id];this.announce(`ENVIRONMENTAL FINISHER<br>${title}`,1450);nearby.broken=true;nearby.active=false;this.property+=900;addEvent(this,`${v.name} received the ${title}.`)}
